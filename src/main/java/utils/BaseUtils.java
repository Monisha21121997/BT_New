  ...
  public void checkURL(String linkURL){
    try{
      ...
      if (connection.getResponseCode() >= 400){
        log.error("--------------------------------------------------------------");
        log.error(givenurl+" ---> "+ connection.getResponseCode()+ connection.getResponseMessage() + " ---> is a broken link");
        log.error("--------------------------------------------------------------\n\n");
      }else {
        log.info("--------------------------------------------------------------");
        log.info(givenurl+" ---> "+connection.getResponseCode()+connection.getResponseMessage());
        log.info("--------------------------------------------------------------\n\n");
      }
    } catch (Exception e) {
