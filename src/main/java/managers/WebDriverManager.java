...
/**
 * This class is responsible to get us the WebDriver, when we ask for it. Only exposes two methods
 * getDriver() and closeDriver(). GetDriver method will decide if the driver is already created or
 * needs to be created. closeDriver method will terminate the created driver. Selenium WebDriver is
 * not thread-safe by default. A variable should be accessed by same thread by which it is created,
 * we can use ThreadLocal variables. This class makes variables thread safe & parallel execution can
